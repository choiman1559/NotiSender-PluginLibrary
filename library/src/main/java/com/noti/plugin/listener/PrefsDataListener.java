...
public class PrefsDataListener {
    ...
    public static void callOnDataReceived(String key, String data) {
        if(m_onReceivedListener != null) {
            for (onReceivedListener listener : m_onReceivedListener) {
                listener.onReceive(key, data);
            }
        }
    }
